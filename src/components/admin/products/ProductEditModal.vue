<template>
  ...
  >
    <div class="w-full max-w-6xl bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
      <!-- Header -->
      <div class="bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-4 text-white flex-shrink-0">
              ...
              <label class="flex items-center gap-2 cursor-pointer group">
                <div class="relative">
                  <input
                    ...
                    class="sr-only peer"
                  />
                  <div class="w-11 h-6 bg-gray-300 rounded-full peer-checked:bg-orange-500 transition"></div>
              ...
              <label class="flex items-center gap-2 cursor-pointer group">
                <div class="relative">
                  <input
                    ...
                    class="sr-only peer"
                  />
                  <div class="w-11 h-6 bg-gray-300 rounded-full peer-checked:bg-green-500 transition"></div>
                  <div class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition peer-checked:translate-x-5"></div>
                </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div v-if="editImages.length" class="grid grid-cols-4 gap-3 mb-3">
                <div v-for="(url, index) in editImages" :key="'old-' + index" class="relative group">
                  <img :src="url" class="w-full h-20 object-cover rounded-lg border-2 border-gray-200" />
                  <button
                    type="button"
              ...
              >
                <svg class="w-10 h-10 text-orange-400 group-hover:text-orange-500 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span class="text-sm font-semibold text-orange-500 group-hover:text-orange-600">Add new images</span>
              ...
              <div v-if="newImagePreviews.length" class="grid grid-cols-4 gap-3 mt-3">
                <div v-for="(preview, index) in newImagePreviews" :key="'new-' + index" class="relative group">
                  <img :src="preview" class="w-full h-20 object-cover rounded-lg border-2 border-green-200" />
                  <div class="absolute top-1 left-1 bg-green-500 text-white text-xs px-1.5 py-0.5 rounded">NEW</div>
                  <button
                    type="button"
            ...
            <div>
              ...
              <div v-if="oldFlagUrl" class="relative w-24 mb-3 group">
                <img :src="oldFlagUrl" class="w-24 h-16 object-cover rounded-lg border-2 border-gray-200 shadow-sm" />
                <button
                  type="button"
                  @click="removeOldFlag"
                  class="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 hover:bg-red-600 text-white text-sm font-bold flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition"
              ...
              <div v-if="flagPreview" class="relative w-24 mb-3 group">
                <img :src="flagPreview" class="w-24 h-16 object-cover rounded-lg border-2 border-green-200 shadow-sm" />
                <div class="absolute top-1 left-1 bg-green-500 text-white text-xs px-1.5 py-0.5 rounded">NEW</div>
                <button
                  type="button"
                  @click="removeFlag"
              ...
              >
                ...
                <span class="text-sm font-semibold text-orange-500 group-hover:text-orange-600">Upload new flag</span>
